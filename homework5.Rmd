...
```{r ans66}
# df_records$township
df_records_unlist <- unlist(df_records$Address)
township <- str_extract(df_records_unlist, 
          pattern = "(?<=[縣市])[\u4E00-\u9FFF][\u4E00-\u9FFF][區鎮鄉市](?<=[區鎮鄉市])")

df_records$township <- as.data.frame(township)
df_records$township
```
